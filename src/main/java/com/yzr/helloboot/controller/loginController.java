...
public class loginController {
	...
	public String dengru(String userName ,String loginPwd,HttpServletRequest request,HttpServletResponse response)throws Exception {
		if (userName==null) {
		 ...
		 Uuuuuuser uuuuuuser =null;
		 try {
			 uuuuuuser =loginService.selectByUserName(userName);
			 //验证用户
			 if(uuuuuuser ==null) {
			 ...
			 //验证密码是否正确
			 if(!MD5Util.getMD5(loginPwd).equals(uuuuuuser.getLoginPwd())) {
			 ...
			 //将用户放入session
			// request.getSession().setAttribute("user", uuuuuuser);
			 return "mainFrm";
		} catch (Exception e) {
			e.printStackTrace();
			return "error";
